...
from app.services.claude_client import _MODEL  # noqa: E402

PROBE_EMAIL = "receipt-probe@local.test"
# A spread of categories so category_suggestion has something real to choose
# between — with a single category the "suggested category" output is vacuous.
PROBE_CATEGORIES = ["Groceries", "Dining Out", "Household", "Transportation", "Entertainment"]
...
async def main() -> int:
    ...
    print(f"  model             {_MODEL}  (from claude_client._MODEL)")
    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            receipt, expense, needs_edit = await receipt_service.process_upload(db, client, family_id, uploader_id, raw)
        ...
        if needs_edit:
            print(
                "\n  \033[33mneeds_edit=True → amount is stored as $0.00 and the UI shows a"
                '\n  "Needs review" chip. Cause: low confidence or no total extracted.\033[0m'
            )
