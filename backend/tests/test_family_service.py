...
async def test_create_family_success(db_session: AsyncSession) -> None:
    ...
    family = await create_family(uow, user, name="Smith Family", timezone="America/Chicago")
...
async def test_create_family_user_already_in_family(db_session: AsyncSession) -> None:
    ...
    await create_family(uow, user, name="First Family")
...
async def test_create_family_default_timezone(db_session: AsyncSession) -> None:
    ...
    family = await create_family(uow, user, name="Default TZ Family")
...
async def test_get_family_with_members(db_session: AsyncSession) -> None:
    ...
    fetched = await get_family_with_members(uow, family.id)
...
async def test_get_family_with_members_not_found(db_session: AsyncSession) -> None:
    ...
    with pytest.raises(HTTPException) as exc_info:
        await get_family_with_members(uow, non_existent_id)
...
async def test_invite_user_nonexistent_email_succeeds_silently(db_session: AsyncSession) -> None:
    ...
    family = await create_family(uow, inviter, name="Invite Family")

    # No user with this email exists — must not raise
    await invite_user(uow, family.id, "nobody@nowhere.invalid", inviter)
...
async def test_invite_user_already_in_family_succeeds_silently(db_session: AsyncSession) -> None:
    ...
    inviter = await create_test_user(db_session)
    family = await create_family(uow, inviter, name="Invite Family 2")
    ...
    target_family = await create_family(uow, target, name="Target Family")
    _ = target_family  # suppress unused-variable warning

    # Must not raise even though user is already in a family
    await invite_user(uow, family.id, "already@example.com", inviter)
...
async def test_invite_user_already_has_pending_invite_succeeds_silently(db_session: AsyncSession) -> None:
    ...
    inviter = await create_test_user(db_session)
    family = await create_family(uow, inviter, name="Invite Family 3")
    target = await create_test_user(db_session, email="pending@example.com")

    # First invite — should succeed and create an invite
    await invite_user(uow, family.id, "pending@example.com", inviter)
...
async def test_invite_user_valid_email_creates_invite(db_session: AsyncSession) -> None:
    ...
    inviter = await create_test_user(db_session)
    family = await create_family(uow, inviter, name="Invite Family 4")
    target = await create_test_user(db_session, email="eligible@example.com")

    # Must not raise when user is eligible
    await invite_user(uow, family.id, "eligible@example.com", inviter)
...
async def test_respond_to_invite_accept_already_in_family_409(db_session: AsyncSession) -> None:
    ...
    other_family = await create_family(uow, other_owner, name="Other Family")
    member = FamilyMember(family_id=other_family.id, user_id=target.id, role="member")
    db_session.add(member)
    await db_session.flush()

    with pytest.raises(HTTPException) as exc_info:
        await respond_to_invite(uow, invite.id, target, "accept")
...
async def test_remove_member_success(db_session: AsyncSession) -> None:
    ...
    owner = await create_test_user(db_session, display_name="Owner")
    family = await create_family(uow, owner, name="Remove Family")
    ...
    await remove_member(uow, family.id, target.id, owner)
...
async def test_remove_member_owner_blocked_403(db_session: AsyncSession) -> None:
    ...
    with pytest.raises(HTTPException) as exc_info:
        await remove_member(uow, family.id, owner.id, admin2)
...
async def test_change_role_success(db_session: AsyncSession) -> None:
    ...
    result = await change_role(uow, family.id, target.id, "admin", owner)
...
async def test_change_role_demote_owner_blocked_403(db_session: AsyncSession) -> None:
    ...
    with pytest.raises(HTTPException) as exc_info:
        await change_role(uow, family.id, owner.id, "member", admin2)
...
async def test_change_role_demote_last_admin_blocked_403(db_session: AsyncSession) -> None:
    ...
    family = await create_family(uow, creator, name="Last Admin Family")
    ...
    family2 = await create_family(uow, owner2, name="Last Admin Family 2")
    ...
    result = await change_role(uow, family2.id, non_owner_admin.id, "member", owner2)
    ...
    family3 = await create_family(uow, owner3, name="Last Admin Family 3")
    ...
    await change_role(uow, family3.id, solo_admin.id, "member", owner3)

    # Re-promote solo_admin
    await change_role(uow, family3.id, solo_admin.id, "admin", owner3)
    ...
    family4 = await create_family(uow, owner4, name="Last Admin Family 4")
    ...
    with pytest.raises(HTTPException) as exc_info:
        await change_role(uow, family4.id, last_admin.id, "member", owner4)
...
@pytest.mark.asyncio
async def test_leave_family_success(db_session: AsyncSession, uow: SqlAlchemyUnitOfWork) -> None:
    """leave_family removes the member record for a non-owner user."""
    from sqlalchemy import select

    owner = await create_test_user(db_session, display_name="Owner")
    family = await create_family(uow, owner, name="Leave Family")
    ...
    await leave_family(uow, family.id, leaver)
...
@pytest.mark.asyncio
async def test_leave_family_owner_blocked_403(db_session: AsyncSession, uow: SqlAlchemyUnitOfWork) -> None:
    """leave_family raises 403 when the owner tries to leave."""
    owner = await create_test_user(db_session, display_name="Owner")
    family = await create_family(uow, owner, name="Owner Leave Family")
